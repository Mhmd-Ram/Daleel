...
@section('content')
    <div class="reveal mx-auto max-w-lg">
        ...
        <h1 class="text-2xl font-semibold tracking-tight text-stone-900">{{ __('app.admin.new_category') }}</h1>

        <form method="POST" action="{{ route('admin.categories.store') }}" class="mt-8 space-y-5 rounded-2xl border border-stone-200 bg-white p-6">
            @csrf
            <div class="flex flex-col gap-2">
                <label for="name" class="text-sm font-medium text-stone-700">{{ __('app.admin.name') }}</label>
                <input id="name" name="name" type="text" value="{{ old('name') }}" required autofocus
                       placeholder="{{ __('app.admin.category_name_placeholder') }}"
            ...
            <div class="flex items-center gap-3">
                <button type="submit" class="rounded-lg bg-emerald-600 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-emerald-700 active:scale-[0.98]">{{ __('app.admin.create_category') }}</button>
